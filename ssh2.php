<?
    ...
    // $ssh = new \phpseclib\Net\SSH2('199.91.65.87');
	if (!$_COOKIE['key_uri']) {
		setCookie("host",$host,time()*1+3600,"/");
		setCookie("password",$password,time()*1+3600,"/");
		setCookie("key_uri",$key_file,time()*1+3600,"/");
		setCookie("username",$username,time()*1+3600,"/");
	} else {
		$host=$_COOKIE[host];
		$key_uri=$_COOKIE[key_uri];
		$password=$_COOKIE[password];
		$username=$_COOKIE[username];
	}
    $ssh = new \phpseclib\Net\SSH2($host);
	if (!$key_uri) $key_uri='http://cocojs.com/temp/private.ppk';
    $key = new \phpseclib\Crypt\RSA();
    // $key->setPassword('Jaisriganesh`123`');
    $key->setPassword($password);
	$key->loadKey(file_get_contents($key_uri));
	...
	$p1=explode("/",$path);
	$px=array_merge(array("public_html"),$p1);
	array_pop($p1);
	$last=implode("/",$p1);
	if ($_GET[path]) $path .= "/";
	$row = exec('cd '.$path.'; ls  | xargs -n 1 ',$output,$error);
